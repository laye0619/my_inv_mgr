...
def __set_params(context):
    ...
    # set position level 01_params
    context.p_position_FORCE_CAL_POSITION_BY_PE_DAYS = 60  # 用于强制使用pe百分位计算仓位, 以XX天为间隔强制使用peba
    context.p_position_RISK_STD_LENTH = 30  # 风险平价长度参数，以XX天内的价格计算标准差
    context.p_position_DIFF = 0.1  # 仓位触发条件设置：实际仓位和目标仓位之间的差异
    context.p_position_PERIOD = 5  # 估值百分位长度 默认5年
    context.p_position_HIGH_THRESHOLD = 0.98  # 股票仓位最高限
    context.p_position_LOW_THRESHOLD = 0.15  # 股票仓位最低限
    context.p_position_AIM0 = '000061.XSHG'
    context.p_position_MANAGEMENT = {'target_stock_ratio': 0,
                                     'target_stock_amount': 0,
                                     'target_bond_ratio': 0,
                                     'target_bond_amount': 0,
                                     'actual_stock_ratio': 0,
                                     'actual_stock_amount': 0,
                                     'actual_bond_ratio': 0,
                                     'actual_bond_amount': 0}
...
def handle_bar(context, bar_dict):
    __update_position_management(context)
    # 首次建仓
    if not context.fired:
        logger.info('首次建仓...')
        t_ind_amount = context.p_position_MANAGEMENT['target_stock_amount'] * context.p_t_ind_POSITION_RATIO
        order_target_value(context.p_t_ind_AIM0, t_ind_amount)
        inv_by_pe_pb_amount = context.p_position_MANAGEMENT[
                                  'target_stock_amount'] * context.p_inv_by_pe_pb_POSITION_RATIO
        order_target_value(context.p_inv_by_pe_pb_AIM0, inv_by_pe_pb_amount)
        t28_amount = context.p_position_MANAGEMENT['target_stock_amount'] * context.p_t28_POSITION_RATIO
        order_target_value(context.p_t28_AIM0, t28_amount)
        bond_amount = context.p_position_MANAGEMENT['target_bond_amount']
        order_target_value(context.p_position_AIM0, bond_amount)
        context.fired = True
        return

    # 测试现有仓位和目标仓位，确定是否执行再平衡
    if abs(context.p_position_MANAGEMENT['target_stock_ratio'] - context.p_position_MANAGEMENT[
        'actual_stock_ratio']) > context.p_position_DIFF:
        logger.info('仓位条件满足，触发再平衡交易...')
        __rebalance(context)

    if pd.to_datetime(context.now.date()) not in context.p_CHECK_DATE:
        return
    logger.info('每周执行所有策略-仓位控制')
    __trans_tendeccy_ind(context)
    __trans_inv_by_pe_pb(context)
    __trans_tendency28(context)


def __update_position_management(context):
    target_stock_ratio = __calculate_today_target_stock_position_level(context)
    if target_stock_ratio >= context.p_position_HIGH_THRESHOLD:
        target_stock_ratio = context.p_position_HIGH_THRESHOLD
    if target_stock_ratio <= context.p_position_LOW_THRESHOLD:
        target_stock_ratio = context.p_position_LOW_THRESHOLD
    target_bond_ratio = 1 - target_stock_ratio
    target_stock_amount = context.portfolio.total_value * context.p_TOTAL_VALUE_BUFFER * target_stock_ratio
    target_bond_amount = context.portfolio.total_value * context.p_TOTAL_VALUE_BUFFER - target_stock_amount

    actual_stock_ratio = (context.portfolio.total_value * context.p_TOTAL_VALUE_BUFFER - get_position(
        context.p_position_AIM0).market_value) / context.portfolio.total_value * context.p_TOTAL_VALUE_BUFFER
    actual_stock_amount = context.portfolio.total_value * context.p_TOTAL_VALUE_BUFFER - get_position(
        context.p_position_AIM0).market_value
    actual_bond_ratio = get_position(
        context.p_position_AIM0).market_value / context.portfolio.total_value * context.p_TOTAL_VALUE_BUFFER
    actual_bond_amount = get_position(context.p_position_AIM0).market_value

    context.p_position_MANAGEMENT['target_stock_ratio'] = target_stock_ratio
    context.p_position_MANAGEMENT['target_bond_ratio'] = target_bond_ratio
    context.p_position_MANAGEMENT['target_stock_amount'] = target_stock_amount
    context.p_position_MANAGEMENT['target_bond_amount'] = target_bond_amount
    context.p_position_MANAGEMENT['actual_stock_ratio'] = actual_stock_ratio
    context.p_position_MANAGEMENT['actual_stock_amount'] = actual_stock_amount
    context.p_position_MANAGEMENT['actual_bond_ratio'] = actual_bond_ratio
    context.p_position_MANAGEMENT['actual_bond_amount'] = actual_bond_amount


def __calculate_today_target_stock_position_level(context):
    return __get_stock_position_level_by_pe(context, cal_method=2)


def __get_stock_position_level_by_pe(context, cal_method):
    indicators = context.p_index_details.loc[context.p_index_details['strategy'] == 'Indicator', 'index_code']
    pe_percentile = __get_pe_percentile(context, indicators, cal_method)
    # 取所有指数分位数平均
    pe_percentile_mean = np.mean(pe_percentile)
    return 1 - pe_percentile_mean


def __get_pe_percentile(context, indicators, cal_method):
    today_date = context.now.date()
    # 取5年数据
    start_date = today_date - timedelta(context.p_position_PERIOD * 365)
    result_list = []
    for code in indicators:
        pe_file_name = ('sh' if
                        context.p_index_details.loc[context.p_index_details['index_code'] == code, 'index_mkt'].iloc[
                            0] == 'SH' else 'sz') + code
        df_pe = pd.read_csv('%s/index_pe_pb/%s_pe_pb.csv' % (utility.DATA_ROOT, pe_file_name))
        # 取5年数据
        df_pe = df_pe.loc[df_pe['Unnamed: 0'].apply(pd.to_datetime) > pd.Timestamp(start_date)]
        df_pe = df_pe.loc[df_pe['Unnamed: 0'].apply(pd.to_datetime) <= pd.Timestamp(today_date)]
        total_count = df_pe['pe%s' % cal_method].count()
        target_pe = df_pe.loc[df_pe['Unnamed: 0'] == str(today_date), 'pe%s' % cal_method]
        small_than_target_count = df_pe.loc[df_pe['pe%s' % cal_method].apply(float) <= float(target_pe.iloc[0])][
            'pe%s' % cal_method].count()
        pe_percentile = small_than_target_count / total_count
        result_list.append(pe_percentile)
    return result_list


# 在平衡交易
def __rebalance(context):
    # 处理position债券基金
    # 首先获取所有货币基金和债券基金
    t28_aim0_value = get_position(context.p_t28_AIM0, POSITION_DIRECTION.LONG).market_value
    t_ind_aim0_value = get_position(context.p_t_ind_AIM0, POSITION_DIRECTION.LONG).market_value
    inv_by_pe_pb_aim0_value = get_position(context.p_inv_by_pe_pb_AIM0, POSITION_DIRECTION.LONG).market_value

    # 然后获取各个策略持仓df
    t28_holding = __get_current_holding(pd.Series([context.p_t28_AIM1, context.p_t28_AIM2]))
    t_ind_holding = __get_current_holding(context.p_t_ind_AIM_LIST)
    inv_by_pe_pb_holding = __get_current_holding(context.p_inv_by_pe_pb_AIM_LIST)

    if context.p_position_MANAGEMENT['target_stock_amount'] - context.p_position_MANAGEMENT[
        'actual_stock_amount'] > 0:  # 需要买股票；卖债券
        order_target_value(context.p_position_AIM0, context.p_position_MANAGEMENT['target_bond_amount'])

        # 以下处理28轮动
        if t28_aim0_value != 0:
            t28_holding = t28_holding.append({'基金代码': context.p_t28_AIM0, '基金现值': t28_aim0_value}, ignore_index=True)
        t28_holding_value = t28_holding['基金现值'].sum()
        delta = (context.p_position_MANAGEMENT[
                     'target_stock_amount'] * context.p_t28_POSITION_RATIO) / t28_holding_value
        for index, row in t28_holding.iterrows():
            order_target_value(row['基金代码'], row['基金现值'] * delta)

        # 以下处理行业轮动
        if t_ind_aim0_value != 0:
            t_ind_holding = t_ind_holding.append({'基金代码': context.p_t_ind_AIM0, '基金现值': t_ind_aim0_value},
                                                 ignore_index=True)
        t_ind_holding_value = t_ind_holding['基金现值'].sum()
        delta = (context.p_position_MANAGEMENT[
                     'target_stock_amount'] * context.p_t_ind_POSITION_RATIO) / t_ind_holding_value
        for index, row in t_ind_holding.iterrows():
            order_target_value(row['基金代码'], row['基金现值'] * delta)

        # 以下处理估值底仓策略
        if inv_by_pe_pb_aim0_value != 0:
            inv_by_pe_pb_holding = inv_by_pe_pb_holding.append(
                {'基金代码': context.p_inv_by_pe_pb_AIM0, '基金现值': inv_by_pe_pb_aim0_value}, ignore_index=True)
        inv_by_pe_pb_holding_value = inv_by_pe_pb_holding['基金现值'].sum()
        delta = (context.p_position_MANAGEMENT[
                     'target_stock_amount'] * context.p_inv_by_pe_pb_POSITION_RATIO) / inv_by_pe_pb_holding_value
        for index, row in inv_by_pe_pb_holding.iterrows():
            order_target_value(row['基金代码'], row['基金现值'] * delta)
    ...
    # 清理现金
    remain_cash = context.config.base.accounts['STOCK'] * (1 - context.p_TOTAL_VALUE_BUFFER)
    to_buy_using_cash = context.portfolio.cash - remain_cash
    if to_buy_using_cash > 0:
        order_value(context.p_position_AIM0, to_buy_using_cash)  # 除了设定的最初预留金额，账户现金都买了债券
